...
def recoverSecret(triplets):
    ...
    for triplet in triplets:
        ...
        # first ensure all triplet letters are in secret
        if third not in secret:
            secret.append(third)
        if second not in secret:
            secret.append(second)
        if first not in secret:
            secret.append(first)

        # organize secret per the rules of the triplet
        # ensure second before third
        if secret.index(second) > secret.index(third):
            secret.insert(secret.index(third),secret.pop(secret.index(second)))
        # ensure first before second
